...
grid1.setInput('size', new Vec2(6, 50));
grid1.setInput('rows', 300);
grid1.setInput('columns', 100);
//box1.setInput('center', new Vec3(0.0, 12, 0.0));
grid1.x = 20;
grid1.y = 20;

const mountain1 = new nodes.GeoMountainNode('mountain1');
mountain1.setExpression(lox, 'offset', 'vec2(0, -$time * 1)');
mountain1.setInput('scale', new Vec2(0.5, 0.3));
...
box1.setInput('size', new Vec3(0.15, 0.05, 0.8));
box1.x = 150;
box1.y = 70;
...
trans1.setInput('translate', new Vec3(0, -0.5, 0));
trans1.setInput('rotate', new Vec3(20, 0, 0));
...
network.connections.push({ outNode: 'grid1', inNode: 'mountain1', inPort: 'geo' });
network.connections.push({ outNode: 'mountain1', inNode: 'merge1', inPort: 'geo1' });
network.connections.push({ outNode: 'box1', inNode: 'merge1', inPort: 'geo2' });
network.connections.push({ outNode: 'merge1', inNode: 'trans1', inPort: 'geo' });
// network.connections.push({ outNode: 'box1', inNode: 'merge1', inPort: 'geo2' });
...
function App() {
  ...
  const animate = () => {
    ...
    const bx = simplex.noise2D(7917, time * 0.1);
    const by = simplex.noise2D(3626, time * 0.1);
    const bz = simplex.noise2D(4643, time * 0.1);
    //const rx = simplex.noise2D(3163, time * 0.02);
    //const ry = simplex.noise2D(3001, time * 0.02);
    const rz = simplex.noise2D(3571, time * 0.1);
    ...
    const transRot = new Vec3(0, 0, remap(rz, -1, 1, -90, 90));
    network.setInput('trans1', 'rotate', transRot);

    const boxCenter = new Vec3(remap(bx, -1, 1, -0.6, 0.6), remap(by, -1, 1, 0.3, 1.3), remap(bz, -1, 1, -1, 3));
    network.setInput('box1', 'center', boxCenter);
