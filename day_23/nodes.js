...
export class CopyAndAffineTransfomNode extends Node {
  constructor(name) {
    super(name, TYPE_SHAPE);
    this.addInput('shape', TYPE_SHAPE);
...
export class AffineTransfomNode extends Node {
  constructor(name) {
    super(name, TYPE_SHAPE);
    this.addInput('shape', TYPE_SHAPE);
    this.addInput('order', TYPE_STRING, 'srt');
...
export class GeoCircleNode extends Node {
  constructor(name) {
    super(name, TYPE_GEO);
    this.addInput('center', TYPE_VEC3, new Vec3(0, 0, 0));
    this.addInput('size', TYPE_VEC2, new Vec3(1, 1));
    this.addInput('segments', TYPE_INT, 30);
  }
...
export class GeoTransformNode extends Node {
  ...
  run() {
    ...
    matrix.premultiply(this._rotateXMatrix);
    matrix.premultiply(this._rotateYMatrix);
    matrix.premultiply(this._rotateZMatrix);
    matrix.premultiply(this._scaleMatrix);
    matrix.premultiply(this._translateMatrix);
    const m = matrix.m;
...
export class GeoCopyToPointsNode extends Node {
  constructor(name) {
    super(name, TYPE_GEO);
    this.addInput('geo', TYPE_GEO);
    this.addInput('target', TYPE_GEO);
  }
  ...
  run() {
    ...
    const newPointCount = geo.points.size * target.points.size;
    const newGeo = new Geo(newPointCount);

    const targetPointCount = target.points.size;
    const txs = target.points.getArray('p[x]');
    const tys = target.points.getArray('p[y]');
    const tzs = target.points.getArray('p[z]');
    // console.log(txs, tys, tzs);
    ...
    for (let i = 0; i < targetPointCount; i++) {
      ...
      for (let j = 0, l = geo.points.size; j < l; j++) {
        ...
        let y = geo.points.get(j, 'p[y]');
        let z = geo.points.get(j, 'p[z]');
        if (pscales) {
          x *= pscales[i];
          y *= pscales[i];
          z *= pscales[i];
        ...
        y += tys[i];
        z += tzs[i];
        newGeo.points.append({ 'p[x]': x, 'p[y]': y, 'p[z]': z });
      }
    }
    this.setOutput(newGeo);
  }
