...
// I want to play with L-systems.
// Expressions in parameters (instead of just values?)
// Maybe for-each nodes to modify the parameters of the L-systems.
...
grid1.setInput('height', 500);

grid1.x = 150;
grid1.y = 20;
const trans1 = new nodes.TransformNode('trans1');
trans1.setInput('shear', new Vec2(1.35, -1));
...
network.connections.push({ outNode: 'lsys1', inNode: 'copy1', inPort: 'shape' });
...
function App() {
  ...
  const animate = () => {
    const time = (Date.now() - startTime) / 750.0;
    const shx = simplex.noise2D(7917, time * 0.01);
    const shy = simplex.noise2D(2833, time * 0.01);
    ...
    network.setInput('lsys1', 'angle', time * 2);
    network.setInput('trans1', 'shear', new Vec2(remap(shx, -1, 1, 1.25, 1.35), remap(shy, -1, 1, -1.5, -0.5)));
